# AGENTS.md — Danfoss Ally Integration
...
1. Implement and maintain the Danfoss Ally integration.
...
- Locate the information inside this repository or from the pydanfossally repository, or
...
- `custom_components/danfoss_ally` — Integration logic
...
When testing integration changes in this repository that depend on in-progress `pydanfossally` module changes, the agent MUST update `custom_components/danfoss_ally/manifest.json` to point to the specific `pydanfossally` branch being used for that test cycle.

When asked to merge, revert the `custom_components/danfoss_ally/manifest.json` requirement changes and check if there is a newer version of the `pydanfossally` module, available from PyPI.
...
## Danfoss Ally Integration Rules
